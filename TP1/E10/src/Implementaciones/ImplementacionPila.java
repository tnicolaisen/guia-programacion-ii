...
public class ImplementacionPila implements Pila {
    ...
    public void Apilar(int elemento) {
        ...
        // Verifico primero que el elemento no exista dentro de la pila
        for (int i = 0; i < cursor; i++) {if (pila[i] == elemento) {encontrado = true;}}

        // En caso de encontrado, agrego elemento. Caso contrario, informo al usuario de la imposibilidad
        if (!encontrado) {
            pila[cursor] = elemento;
            cursor++;
        } else {
            System.out.println("INFO: No se ha podido agregar el elemento " + elemento + " porque ya existe dentro de su pila.");
        }
    }
